...
library("phyloseq"); library("ggplot2"); library("scales"); library("grid"); library("reshape"); library(RColorBrewer); library("xtable")
...
Sequence observations converted to percentage for each sample, to allow easy comparison between samples of different sequencing depth. 
```{r prep_OTUs,echo=FALSE,warning=FALSE}
expt.rel <- transform_sample_counts(expt, function(x) 100*x/(sum(x)))
#rank.names(expt.rel)
#tax_table(expt.rel)[,"Phylum"]=="Actinobacteria"
#tax_table(expt.rel)[tax_table(expt.rel)[,"Phylum"]=="Actinobacteria","Phylum"] <- "Actinobacteria2"
...
### Richness / diversity
Per-sample data was rarefied to an even sequencing depth. A merged object was also made to combine all of the similar sample types for a better estimation of alpha diversity (but without replication). This object was rarefied to the minimum sequencing depth of the set.

```{r prep_diversity,echo=FALSE,warning=FALSE,fig.width=12,fig.height=6}
cat("Summary of read lengths in original quality controlled experiment")
summary(sample_sums(expt))
sort(sample_sums(expt))
plot(sort(sample_sums(expt)))
even_depth <- 437
cat("Rarefaction to depth of",even_depth,"sequences per sample:")
expt.rare <- rarefy_even_depth(expt,sample.size=even_depth,rngseed=42,replace=FALSE,trimOTUs=TRUE)
#sample_sums(expt.rare)

expt.diversity <- expt.rare

# Prepare merged object for richness estimates later
sample_data(this_analysis)$zone_type <- paste(sample_data(this_analysis)$zone_code,sample_data(this_analysis)$type,sep="_")
# merge samples of same type to increase depth of coverage (for richness estimate)
this_analysis.merged <- merge_samples(this_analysis, "zone_type")
sort(sample_sums(this_analysis.merged))
# repair variables that were damaged during merge (coerced to numeric)
sample_data(this_analysis.merged)$zone_type <- factor(sample_names(this_analysis.merged))
sample_data(this_analysis.merged)$zone <- gsub(pattern="_.*",replacement="",x=sample_data(this_analysis.merged)$zone_type)
sample_data(this_analysis.merged)$zone_code <-  gsub(pattern="_.*",replacement="",x=sample_data(this_analysis.merged)$zone_type)
sample_data(this_analysis.merged)$type <- gsub(pattern=".*_",replacement="",x=sample_data(this_analysis.merged)$zone_type)

# rarefy the merged experiment too
cat("Summary of read lengths in original quality controlled experiment - merged by sample type")
summary(sample_sums(this_analysis.merged))
cat("Rarefaction to depth of",summary(sample_sums(this_analysis.merged))[1],"sequences per sample:")

expt.diversity.merged <- rarefy_even_depth(sample.size=summary(sample_sums(this_analysis.merged))[1],this_analysis.merged,rngseed=42,replace=FALSE,trimOTUs=TRUE)
...
even_option <- "FALSE"
if(even_option) {
  expt.temp <- expt.diversity
  cat("Using even sampling depth of",even_depth,"for ordinations")
  cat("NOTE - this option will give the wrong result. Abundance trimming needs to be done based on the original data, not the even depth data. Anyway, results are similar regardless.")
} else {
  expt.temp <- expt
...
# plot of richness vs zone
plot_title <- "Diversity of merged samples"
p_rich_zone <- plot_richness(expt.diversity.merged, x = "zone_code", title=plot_title,color="type",shape="type",measures = alpha_measures) 
p_rich_zone = p_rich_zone + geom_point(size=5, alpha=alpha)
# Remove the original non-transparent, small-size `geom_point` layer.
p_rich_zone$layers = p_rich_zone$layers[-1]
# Set the manual color and shape scales
p_rich_zone = p_rich_zone + scale_colour_manual(values=color_pal,name="Depth") + scale_shape_manual(values=shape_scale,name="Depth")
#p_rich_zone
...
p_rich_zone3 =  p_rich_zone2 + geom_point(data=p_rich_zone$data,mapping=p_rich_zone$mapping,scale=p_rich_zone$scales,theme=p_rich_zone$theme,size=5, alpha=alpha) + geom_errorbar(data=p_rich_zone$data,mapping=p_rich_zone$layers[[1]]$mapping)
#p_rich_zone2
...
write.table(estimate_richness(expt.diversity.merged, split=TRUE), 
paste(tables_dir,"richness_merged_split.txt",sep="/"),sep="\t")

write.table(estimate_richness(expt.diversity.merged, split=FALSE), 
paste(tables_dir,"richness_merged.txt",sep="/"),sep="\t")
...
<a href='figures/OR4[ordination].png'><img src='figures/OR4[ordination].png', width='800'></a>


```{r unifrac,eval=FALSE}
# UNIFRAC is slow
# results reasonably similar to CCA bray
